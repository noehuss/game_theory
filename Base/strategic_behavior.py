...
class MPEC:
    ...
    def constraints(self):
        ...
        def rule_capamin(model, i):
            return model.Pg[i] >= model.Pmin[i]
        self.model.constraint_capamin = Constraint(self.model.I, rule=rule_capamin)

        def rule_pmin(model,i):
            return model.Pg[i] - model.Pmin[i] <= self.bigM*model.z_min[i]
        ...
        def rule_pmax(model, i):
            return model.Pmax[i] - model.Pg[i] <= self.bigM*model.z_max[i]
        self.model.constraint_pgmax = Constraint(self.model.I, rule=rule_pmax)
