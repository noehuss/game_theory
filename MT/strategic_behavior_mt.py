...
class MPEC:
    def __init__(self, producer: str, alphas: list[list], marginal_costs: list, prod_df:pd.DataFrame, demand: list, bigM=1e3, tau=1.0, upper_bid=100, lower_bid=0, set_ramp=True):
        """
        MPEC algorithm
        ...
        self.T = len(demand)-1
        self.set_ramp = 1 if set_ramp is True else 0
        self.indexes()
        self.parameters()
        self.variables()
        self.constraints()
        ...
        def rule_price_balance(model, t, i):
            if self.set_ramp == 0:
                ...
                expr_theta = 0
                 ...
                 expr_theta = 0 #- model.theta_max[t+1, i] + model.theta_min[t+1, i]
            elif t == self.T:
    ...
    def solve(self):
        ...
        self.model.constraint_ramp_up_max.display()
        self.model.Ramp.display()
    ...
    def get_results(self, t) -> pd.DataFrame:
        ...
        data = {
            ...
            'Pmax': [min(value(self.model.Pmax[i]), value(self.model.Pg[t-1, i])+value(self.model.Ramp[i]) if t>0 else 1000) for i in self.model.I], #type: ignore
...
class MPEClinearized(MPEC):
    def objective_function(self):
        ...
        sum_thetas = self.set_ramp*sum(sum((-self.model.theta_max[t, i]+self.model.theta_min[t, i])*self.model.Ramp[i] for i in self.model.OmegaBar) for t in self.model.T)
