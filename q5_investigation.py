...
br = BR(bids_init=[prod_df['marginal_costs'].to_list().copy() for i in range(len(demands))], marginal_costs=[10, 30, 35, 55], demand=demands, prod_df=prod_df, tolerance=0.01, set_ramp=False)
br.run_BR(300)
...
for hour, demand in enumerate(demands):
    print(f"Hour {hour}: Demand = {demand}")
    utils.plot_merit_order(br.get_results(hour)[0], demand=demand, ax_fig=axes[hour])
plt.show()
# %%
...
br = BR(bids_init=[prod_df['marginal_costs'].to_list().copy() for i in range(len(demands))], marginal_costs=[10, 30, 35, 55], demand=demands, prod_df=prod_df, tolerance=0.01, set_ramp=True)
br.run_BR(300)
results_br, as_converged = br.get_results()
price_br = br.get_price()
print("bids: ", br.dict_alphas)
